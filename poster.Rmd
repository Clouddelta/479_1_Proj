...
- Our Kaggle dataset^1^ also came with Geohash boundaries as well as Latitude and Longitude coordinates for each parking area. We utilized this to create a Leaflet of all the parking areas.

<center>![Leaflet of New York](images/newyork.png){height=40%, width=50%}</center>

- We can use this Leaflet to gain a closer look at different cities.
...
- The key goal in Ordinary Kriging is to minimize the error variance. First, we draw a tile plot among states and have a glimpse of how hard it is to park in different states.

<center>![Tile Plot from Ordinary Kriging](images/krig.png){height=100%, width=100%}</center>

- The coefficient is normalized, and deeper color means it is harder to park in this area. We can see that it takes the longest time to find a parking place in New York.
